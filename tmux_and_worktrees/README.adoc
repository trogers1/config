...
Before opening a newly created worktree session, `dtree` runs one startup script.
It uses the first project-specific script it finds in the main checkout:
...
* symlinks Terraform state files (`terraform/**.tfstate` and `terraform/**.tfstate.*`) from
  the main checkout when they exist
