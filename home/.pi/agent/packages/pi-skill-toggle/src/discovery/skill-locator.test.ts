...
describe("DefaultSkillLocator", () => {
  ...
  it("finds global, user, and project skills with Pi root markdown discovery rules", async () => {
    ...
    assert.equal(
      byPath.get("/home/tester/.pi/agent/skills/user-root.md")?.source.kind,
      "user",
    );
    assert.equal(
      byPath.get("/home/tester/.agents/skills/global-skill/SKILL.md")?.source
        .kind,
      "global",
    );
    assert.equal(
      byPath.get("/repo/.pi/skills/project-root.md")?.source.kind,
      "project",
    );
    assert.equal(
      byPath.get("/repo/.agents/skills/project-legacy-skill/SKILL.md")?.source
        .kind,
      "project-legacy",
    );
    assert.equal(
      byPath.has("/home/tester/.agents/skills/ignored-global-root.md"),
      false,
    );
    assert.equal(
      byPath.has("/repo/.agents/skills/ignored-project-legacy-root.md"),
      false,
    );
  });
});
...
class MemoryTreeFileSystem implements FileSystem {
  ...
  async stat(
    path: string,
  ): Promise<{ isDirectory: boolean; isFile: boolean; mode: number }> {
    return {
