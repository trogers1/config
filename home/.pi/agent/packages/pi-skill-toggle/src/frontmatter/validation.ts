...
export function hasDuplicateDisableModelInvocation(
  doc: FrontmatterDocument,
): boolean {
  return (
    (getTopLevelFrontmatterKeyCounts(doc).get("disable-model-invocation") ??
      0) > 1
  );
}

export function getDuplicateFrontmatterKeys(
  doc: FrontmatterDocument,
): string[] {
...
function getTopLevelFrontmatterKeyCounts(
  doc: FrontmatterDocument,
): Map<string, number> {
