...
export class MinimalFrontmatterPatcher implements FrontmatterPatcher {
  patchInvocationMode({
    ...
    const newFrontmatter =
      desiredMode === "manual-only"
        ? ensureManualOnly({
            frontmatterText: doc.frontmatterText,
...
function ensureManualOnly({
  ...
  const lines = splitLinesPreserve({
...
function ensureAgentInvocable(frontmatterText: string): string {
  return splitLinesPreserve({
    text: frontmatterText,
    fallbackEol: frontmatterText.includes("\r\n") ? "\r\n" : "\n",
  })
    .filter((line) => !DISABLE_KEY_RE.test(stripEol(line)))
    .join("");
}

function splitLinesPreserve({
  text,
  fallbackEol,
}: {
  text: string;
