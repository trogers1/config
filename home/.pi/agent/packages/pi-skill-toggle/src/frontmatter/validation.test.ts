...
describe("frontmatter validation", () => {
  it("reports duplicate top-level frontmatter keys", () => {
    ...
    assert.deepEqual(getDuplicateFrontmatterKeys(doc), [
      "disable-model-invocation",
    ]);
    assert.equal(hasDuplicateDisableModelInvocation(doc), true);
    assert.deepEqual(
      deriveSkillMetadata({ filePath: "/skills/handoff/SKILL.md", doc })
        .diagnostics,
      [
        {
          severity: "warning",
          message: "Duplicate frontmatter key: disable-model-invocation",
        },
      ],
    );
  });
