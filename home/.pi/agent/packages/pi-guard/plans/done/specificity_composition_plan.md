...
1. `test(pi-guard): specify specificity-based composition (failing tests)`
2. `feat(pi-guard): resolve rules by specificity with order tiebreak`
3. `refactor(pi-guard): decompose shipped profiles into ruleSets.lib and profiles.lib`
4. `feat(pi-guard): multi-extends and policy transforms`
5. `feat(pi-guard): unify protected paths as composable rules`
6. `feat(pi-guard): expose rule sets via the ruleset: namespace`
7. `feat(pi-guard): add committer/reviewer/scribe-only/deps-mutator/no-shell/implementation-only/git-full profiles`
8. `feat(pi-guard): permissions explain command and docs`
