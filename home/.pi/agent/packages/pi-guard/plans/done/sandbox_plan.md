...
Add optional kernel-enforced containment to `pi-guard` so every approved
...
- `extensions/guard.ts` owns active-profile selection, profile commands,
...
`extensions/guard.ts` into `modules/subagentScopes.ts`. The shared result
feeds:
