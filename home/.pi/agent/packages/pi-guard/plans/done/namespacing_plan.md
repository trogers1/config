...
- Public type/API fixtures compile from `pi-guard/config` or the documented profiles entry point.
...
```jsonc
{
  "$schema": "https://example.com/pi-guard/profiles.schema.json",
  "defaultProfile": "client-work",
  "profiles": {
    "client-work": {
      "extends": "builtin:default",
...
1. `test(pi-guard): define reserved builtin profile namespace`
2. `feat(pi-guard): namespace shipped profiles`
3. `fix(pi-guard): separate builtin and custom profile resolution`
4. `fix(pi-guard): isolate profile registries per extension instance`
5. `feat(pi-guard): reserve builtin names in profile schema`
6. `chore(pi-guard): update profile selectors and fixtures`
7. `docs(pi-guard): document builtin profile namespace`
