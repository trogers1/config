...
- `/profile-add` walks through creating a custom profile: use the searchable picker to compose profiles and rulesets (type to search; ↑/↓ and Return choose; select `✅ Done` to finish), optionally apply a transform, add protected-path rules, choose Bash sandboxing, and enter the profile metadata. Composition and protected-path rules resolve with later entries winning equal-specificity ties. The protected-path editor starts as `⛔️ <glob>`; Tab toggles deny/allow, Enter adds the rule, and Esc skips it. Choose `Add another protected path` or `Continue` after each rule. Examples include `⛔️ .env`, `⛔️ **/credentials/**`, and `✅ .env.example`. Emoji is optional and defaults to `💅`; custom profiles use purple text by default. The saved profile is activated immediately.
- For a Bash ASK request, choose `No (default)` to reject it, `Yes` to allow it once, or `Update profile with choice` to edit and save rules. The update flow can target Bash patterns, protected path globs, or both. Its Bash editor allows manual pattern edits and optional deny steering: ↑/↓ moves between rows, Tab cycles skip/deny/allow, Enter saves, and Esc cancels. `Ctrl+Shift+R` resets the editor to the original patterns with every choice skipped; it is an editor reset, not a profile reset.
