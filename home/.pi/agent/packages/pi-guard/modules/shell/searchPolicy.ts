...
export function injectGrepProtectedPathGlob(
  ...
  return [
    ...
    "Pi's built-in grep forwards only one --glob to ripgrep, so a caller glob can re-include protected files. For filtered searches, use Bash + rg instead (for example, rg --glob '**/*.ts' 'PATTERN' .); pi-guard appends the protected exclusions automatically. Otherwise, search an explicit configured exception or omit glob to apply the profile-derived exclusions automatically.",
  ].join("\n\n");
}
