...
describe("subagent tool", () => {
	...
	const originalParentProfile = process.env.PI_GUARD_ACTIVE_PROFILE;
	...
	afterEach(() => {
		...
		if (originalParentProfile === undefined) delete process.env.PI_GUARD_ACTIVE_PROFILE;
		else process.env.PI_GUARD_ACTIVE_PROFILE = originalParentProfile;
		delete process.env.PI_SUBAGENT_DEPTH;
	});
	...
	it("applies the parent's active permissions profile to a worker", async () => {
		const projectDir = setupProjectDir();
		process.env.PI_GUARD_ACTIVE_PROFILE = "personal";
	...
	it("reports pi-guard-blocked worker actions in the result and handoff", async () => {
		const projectDir = setupProjectDir();
		...
		const result = await runSingle(
			projectDir,
			{
				...
				permissionBlock: { toolName: "write", reason: "[⛔️ by pi-guard] write denied by policy for path: .env" },
			},
			{ agent: "worker", task: "Try to update config", runDir },
		);

		expect(getToolResultText(result)).toContain(
			"⛔ BLOCKED BY PI-GUARD (write): [⛔️ by pi-guard] write denied by policy",
		);
		...
		expect(details.results[0].permissionBlocks).toEqual([
			{ toolName: "write", reason: "[⛔️ by pi-guard] write denied by policy for path: .env" },
		]);
		const handoff = readFileSync(join(runDir, "handoff-worker-try-to-update-config.md"), "utf-8");
		expect(handoff).toContain("⛔ Blocked by pi-guard");
	});
