...
export type UsageCommand =
  ...
  | { kind: "import" }
  | { kind: "export"; path?: string; range: Range; provider?: string; model?: string };
...
export function parseUsageArgs(raw: string, now = Date.now()): UsageCommand {
  ...
  let range = lastDaysRange(7, "last 7 days", now);
  ...
  while (index < tokens.length) {
    ...
    if (token === "today") {
      ...
      if (token === "7") index++;
      range = lastDaysRange(7, "last 7 days", now);
    } else if (token === "month" || token === "30d" || token === "1m" || (token === "1" && tokens[index] === "month")) {
      if (token === "1") index++;
      range = lastDaysRange(30, "last 30 days", now);
    } else if (token === "since") {
      const date = tokens[index++];
      if (!date) throw new Error(usageHelp);
      range = sinceRange(date, now);
    ...
    } else if (kind === "export" && !exportPath) {
      exportPath = token;
    } else {
  ...
  if (kind === "export") return { kind: "export", path: exportPath, range, provider, model };
  return { kind: "report", range, provider, model };
}

function todayRange(now: number): Range {
  const start = new Date(now);
  start.setHours(0, 0, 0, 0);
  return { label: "today", startMs: start.getTime(), endMs: now + 1 };
}

function lastDaysRange(days: number, label: string, now: number): Range {
  const start = new Date(now);
  start.setHours(0, 0, 0, 0);
...
function sinceRange(dateText: string, now: number): Range {
  ...
  return { label: `since ${dateText}`, startMs: start.getTime(), endMs: now + 1 };
}
