...
- **Permissions**: each agent starts with a `pi-permissions` profile (`builtin:read-only` for `scout`/`planner`/`reviewer`, `builtin:worker` for `worker`), and declared `writes` scopes are exported as `PI_SUBAGENT_PERMISSIBLE_GLOBS` for hard enforcement. Because workers are non-interactive, confirmation-gated (`ask`) commands are **blocked**. If a worker hits a permission wall, either run the command yourself in the main session or broaden the agent/task permissions, then resume the worker via `sessionId`.
